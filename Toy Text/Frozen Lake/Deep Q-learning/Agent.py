...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        for state in batch.state:
            onehot = np.zeros(self.state_dim)    # FrozenLake有16个状态
            onehot[state] = 1.0
            states_onehot.append(onehot)
        
        for next_state in batch.next_state:
            onehot = np.zeros(self.state_dim)    # FrozenLake有16个状态
            onehot[next_state] = 1.0
            next_states_onehot.append(onehot)

        states = torch.FloatTensor(states_onehot)
        actions = torch.LongTensor(batch.action)
        rewards = torch.FloatTensor(batch.reward)
        next_states = torch.FloatTensor(next_states_onehot)
...
class DQNAgent:
    ...
    def state_to_onehot(self, state):
        if isinstance(state, (int, np.integer)):
            ...
            onehot = np.zeros((batch_size, self.state_dim))
    ...
    def act(self, state):
        ...
        # 贪婪动作
        state_onehot = np.eye(self.state_dim)[state]
        state_tensor = torch.FloatTensor(state_onehot).unsqueeze(0).to(device)
        with torch.no_grad():
            q_values = self.q_net(state_tensor)    # 计算Q值
    ...
    def train(self, env, config):
        ...
        for episode in range(config.train_episodes):
            ...
            # 每10个episode评估一次
            if (episode + 1) % 10 == 0:
                avg_eval_score = self.evaluate(env, episodes=20)
